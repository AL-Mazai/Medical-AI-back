...
# 提取灰度特征
# @jit
def get_gray_feature(c_features):
    # 灰度特征提取算法
...
# 提取形态特征，如肿瘤的面积、周长、重心等
def get_geometry_feature(c_features):
    # 形态特征  分割mask获得一些特征
...
# 提取肿瘤特征,整合形态特征、灰度特征和纹理特征的提取过程
def get_feature(image, mask, c_features):
    global w
    ...
    get_geometry_feature(c_features)

    get_gray_feature(c_features)
...
def main(pid):
    ...
    get_feature(f'tmp/ct/{pid}.dcm', f'tmp/mask/{pid}_mask.png', c_features)
