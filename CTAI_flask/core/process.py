...
def pre_process(data_path):
    """预处理：首先读取dcm文件，然后对图像数据进行处理，包括选择感兴趣区域（ROI）、归一化、转换为PyTorch张量等。
    最后，将处理后的图像数据保存为PNG格式的图片文件。函数返回处理后的图像数据以及文件名"""
...
def last_process(file_name):
    """对模型输出的分割结果进行后处理。它读取保存的原始图像文件和分割结果掩码文件，然后使用OpenCV的cv2.findContours函数找到掩码中的轮廓，
    并在原始图像上绘制这些轮廓，最后将结果保存为PNG格式的图片文件。"""
